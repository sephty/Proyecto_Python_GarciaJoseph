...
import controllers.listar as buscar
...
def opcion_2():
    ...
    if "libros" in elementos_temporales and elementos_temporales["libros"]:
        for libro in elementos_temporales["libros"]:
            all_elements.append({
                "Tipo": "Libro",
                "Título": libro["titulo"],
                "Autor/Director/Artista": libro["autor"],
                "Género": libro["genero"],
    ...
    if "peliculas" in elementos_temporales and elementos_temporales["peliculas"]:
        for pelicula in elementos_temporales["peliculas"]:
            all_elements.append({
                "Tipo": "Película",
                "Título": pelicula["titulo"],
                "Autor/Director/Artista": pelicula["director"],
                "Género": pelicula["genero"],
                "Valoración": pelicula["valoracion"],
                "Año": pelicula["anio"],
            })
    if "musica" in elementos_temporales and elementos_temporales["musica"]:
        for musica in elementos_temporales["musica"]:
            all_elements.append({
                "Tipo": "Música",
                "Título": musica["titulo"],
                "Autor/Director/Artista": musica["artista"],
                "Género": musica["genero"],
                "Valoración": musica["valoracion"],
                "Año": musica.get("anio", "N/A"),
            })
    if not all_elements:
        print("No hay elementos para mostrar.")
    else:
        print(tabulate(all_elements, headers="keys", tablefmt="pretty"))
